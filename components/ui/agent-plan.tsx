...
const initialTasks: Task[] = [
  ...
  },
  {
    ...
    description: "Execute the first development sprint based on plan",
    status: "pending",
    priority: "medium",
    level: 1,
    dependencies: ["4"],
    subtasks: [
      {
        ...
        description:
          "Develop the essential features identified in requirements",
        status: "pending",
        priority: "high",
...
export default function Plan({ tasks: customTasks }: PlanProps) {
  ...
  return (
    <div className="text-slate-950 dark:text-slate-50 w-full">
      <motion.div 
                    ...
                    >
                        ...
                        <AnimatePresence mode="wait">
                          ...
                          >
                            {task.status === "completed" ? (
                              ...
                              <CircleDotDashed className="h-4.5 w-4.5 text-blue-500 animate-spin" style={{ animationDuration: "8s" }} />
                            ) : task.status === "need-help" ? (
                              <AlertCircle className="h-4.5 w-4.5 text-yellow-500" />
                            ) : task.status === "failed" ? (
                              <XCircle className="h-4.5 w-4.5 text-red-500" />
                            ) : (
                              <Circle className="text-slate-500 h-4.5 w-4.5 dark:text-slate-400" />
                            )}
                          </motion.div>
                        </AnimatePresence>
                      </motion.div>
                      ...
                      >
                        <div className="mr-2 flex-1 truncate">
                          <span
                            className={`${isCompleted ? "text-slate-500 line-through dark:text-slate-400" : ""}`}
                          >
                            {task.title}
                          </span>
                        </div>
                            ...
                            <div className="flex items-center mr-2">
                              <div className="flex flex-wrap gap-1">
                                {task.dependencies.map((dep, idx) => (
                                  <motion.span
                                    key={idx}
                                    className="bg-slate-100/40 text-slate-900 rounded px-1.5 py-0.5 text-[10px] font-medium shadow-sm dark:bg-slate-800/40 dark:text-slate-50"
                    ...
                    <AnimatePresence mode="wait">
                      {isExpanded && task.subtasks.length > 0 && (
                        ...
                        >
                          ...
                          <div className="absolute top-0 bottom-0 left-[20px] border-l-2 border-dashed border-slate-500/30 dark:border-slate-400/30" />
                          <ul className="border-slate-100 mt-1 mr-2 mb-1.5 ml-3 space-y-0.5 dark:border-slate-800">
                              ...
                              return (
                                ...
                                >
                                  ...
                                  >
                                      ...
                                      <AnimatePresence mode="wait">
                                        ...
                                        >
                                          {subtask.status === "completed" ? (
                                            ...
                                            <CircleDotDashed className="h-3.5 w-3.5 text-blue-500 animate-spin" style={{ animationDuration: "8s" }} />
                                          ) : subtask.status === "need-help" ? (
                                            <AlertCircle className="h-3.5 w-3.5 text-yellow-500" />
                                          ) : subtask.status === "failed" ? (
                                            <XCircle className="h-3.5 w-3.5 text-red-500" />
                                          ) : (
                                            <Circle className="text-slate-500 h-3.5 w-3.5 dark:text-slate-400" />
                                          )}
                                        </motion.div>
                                      </AnimatePresence>
                                    </motion.div>
                                    ...
                                    <span
                                      className={`cursor-pointer text-sm ${subtask.status === "completed" ? "text-slate-500 line-through dark:text-slate-400" : ""}`}
                                    >
                                      {subtask.title}
                                    </span>
                                  </motion.div>
